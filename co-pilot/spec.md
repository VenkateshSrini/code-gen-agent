...
Build a RESTful API in python for managing tasks with support for user authentication, task CRUD operations, task categorization, and task filtering.
...
- Monolithic FastAPI application with layered services architecture
...
- Kubernetes deployment in real-time
- Deployable to any Kubernetes-compatible platform (EKS)
- Health checks for container orchestration

**Email Verification**:
- Development: Use console/file-based email backend (no real SMTP)
- Production: Configurable SMTP via environment variables
- Email verification tokens stored in database with expiration
- Stub implementation for local testing (emails logged to console)

**Caching Strategy**:
- Redis for session/token caching
- Cache user data for 15 minutes to reduce DB queries
...
**Token Storage**:
- Access tokens: Short-lived (1 hour), stored in memory/Redis
- Refresh tokens: Long-lived (7 days), stored in database with user association
...
All generated artifacts from the implementation workflow must be organized as follows:
...
├── implementation.md        # Full implementation details (generated)
└── outputs/                 # Generated source code files
    └── src/                 # Extracted code from implementation
        ├── main.py
        ├── models/
        ├── routes/
        ├── services/
...
**Requirements**:
- All markdown outputs (plan.md, tasks.md, implementation.md) must be stored in the **same directory as spec.md**
- Generated source code files must be extracted to `outputs/src/`
- The `outputs/` directory is only for source code, not .md files
- Directory structure must be created automatically if it doesn't exist
...
7. All generated artifacts properly organized (.md files in spec directory, code in outputs/src/)
